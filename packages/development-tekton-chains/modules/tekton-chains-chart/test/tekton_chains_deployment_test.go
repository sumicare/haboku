...
var _ = Describe("TektonChains Deployment", func() {
	var (
	...
	BeforeEach(func() {
		...
		namespace = "kube-system" // Default namespace for tekton-chains
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "tekton-chains", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("tekton-chains"))
			}),
		...
		It("should use correct tekton-chains version from package.json", func() {
			// Verify tekton-chains version is set
			Expect(terraformOptions.Vars).To(HaveKey("tekton-chains_version"))
			version := terraformOptions.Vars["tekton-chains_version"]
