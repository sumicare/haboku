...
var _ = Describe("Tekton Trigger Deployment", func() {
	var (
	...
	BeforeEach(func() {
		...
		namespace = "kube-system" // Default namespace for tekton-trigger
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "tekton-trigger", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("tekton-trigger"))
			}),
		...
		It("should use correct tekton-trigger version from package.json", func() {
			// Verify tekton-trigger version is set
			Expect(terraformOptions.Vars).To(HaveKey("tekton-trigger_version"))
			version := terraformOptions.Vars["tekton-trigger_version"]
		...
		DescribeTable("should validate pod properties",
			...
			Entry("has pods running", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				...
				pods, err := cs.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{
					LabelSelector: "app.kubernetes.io/part-of=tekton-trigger",
				})
				Expect(err).NotTo(HaveOccurred())
