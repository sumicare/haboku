...
resource "docker_image" "postgresql" {
  name         = "${var.repository}${var.org}/postgresql:${var.postgresql_version}"
  keep_locally = true
  triggers = {
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/postgresql:${var.postgresql_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "postgresql" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.postgresql.name
  keep_remotely = true
  ...
  depends_on = [docker_image.postgresql]
}
