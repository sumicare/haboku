...
var _ = Describe("ArgoCD Deployment", func() {
	...
	BeforeEach(func() {
		...
		namespace = "kube-system" // Default namespace for argo-events
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "argo-events", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("argo-events"))
			}),
		...
		It("should use correct argo-events version from package.json", func() {
			// Verify argo-events version is set
			Expect(terraformOptions.Vars).To(HaveKey("argo-events_version"))
			version := terraformOptions.Vars["argo-events_version"]
		...
		DescribeTable("should validate pod properties",
			...
			Entry("has pods running", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				...
				pods, err := cs.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{
					LabelSelector: "app.kubernetes.io/part-of=argo-events",
				})
				Expect(err).NotTo(HaveOccurred())
