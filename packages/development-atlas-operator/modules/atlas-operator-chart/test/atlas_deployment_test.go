...
var _ = Describe("AtlasOperator Deployment", func() {
	var (
	...
	BeforeEach(func() {
		...
		namespace = "kube-system" // Default namespace for atlas-operator
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "atlas-operator", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("atlas-operator"))
			}),
		...
		It("should use correct atlas-operator version from package.json", func() {
			// Verify atlas-operator version is set
			Expect(terraformOptions.Vars).To(HaveKey("atlas-operator_version"))
			version := terraformOptions.Vars["atlas-operator_version"]
		...
		DescribeTable("should validate pod properties",
			...
			Entry("has pods running", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				...
				pods, err := cs.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{
					LabelSelector: "app.kubernetes.io/part-of=atlas-operator",
				})
				Expect(err).NotTo(HaveOccurred())
