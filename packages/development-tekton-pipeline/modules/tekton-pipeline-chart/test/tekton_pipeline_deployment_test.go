...
var _ = Describe("TektonPipeline Deployment", func() {
	var (
	...
	BeforeEach(func() {
		...
		namespace = "kube-system" // Default namespace for tekton-pipeline
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "tekton-pipeline", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("tekton-pipeline"))
			}),
		...
		It("should use correct tekton-pipeline version from package.json", func() {
			// Verify tekton-pipeline version is set
			Expect(terraformOptions.Vars).To(HaveKey("tekton-pipeline_version"))
			version := terraformOptions.Vars["tekton-pipeline_version"]
		...
		DescribeTable("should validate pod properties",
			...
			Entry("has pods running", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				...
				pods, err := cs.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{
					LabelSelector: "app.kubernetes.io/part-of=tekton-pipeline",
				})
				Expect(err).NotTo(HaveOccurred())
