...
var _ = Describe("Apache DataFusion Ballista Deployment", func() {
	var (
	...
	BeforeEach(func() {
		...
		namespace = "kube-system" // Default namespace for datafusion-ballista
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "datafusion-ballista", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("datafusion-ballista"))
			}),
		...
		It("should use correct datafusion-ballista version from package.json", func() {
			// Verify datafusion-ballista version is set
			Expect(terraformOptions.Vars).To(HaveKey("datafusion_ballista_version"))
			version := terraformOptions.Vars["datafusion_ballista_version"]
		...
		DescribeTable("should validate pod properties",
			...
			Entry("has pods running", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				...
				pods, err := cs.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{
					LabelSelector: "app.kubernetes.io/part-of=datafusion-ballista",
				})
				Expect(err).NotTo(HaveOccurred())
