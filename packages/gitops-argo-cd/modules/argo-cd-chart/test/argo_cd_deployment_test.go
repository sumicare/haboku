...
var _ = Describe("ArgoCD Deployment", func() {
	var (
	...
	BeforeEach(func() {
		if !isDeploymentTestEnabled() {
			Skip("Skipping deployment tests (set TEST_DEPLOYMENT=1 to enable)")
		}

		ctx = context.Background()
		namespace = "kube-system" // Default namespace for argocd
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "argocd", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("argocd"))
			}),
		...
		It("should use correct argocd version from package.json", func() {
			// Verify argocd version is set
			Expect(terraformOptions.Vars).To(HaveKey("argocd_version"))
			version := terraformOptions.Vars["argocd_version"]
...
// TestChartYAMLSnapshot tests chart YAML output against golden file.
func TestChartYAMLSnapshot(t *testing.T) {
	if !isDeploymentTestEnabled() {
		t.Skip("Skipping snapshot test (set TEST_DEPLOYMENT=1 to enable)")
	}

	chartYAML, err := os.ReadFile(chartYAMLFile)
	if err != nil {
		t.Fatalf("Failed to read chart YAML file: %v", err)
	}

	if len(chartYAML) == 0 {
		t.Fatal("Chart YAML is empty")
	}
	...
	golden.Assert(t, "argo-cd-chart", chartYAML)
}
