...
const (
	tofuBinary    = "tofu"
	chartYAMLFile = "testdata/chart_output.yaml"
)
...
var (
	//nolint:gochecknoglobals // test suite state
	terraformOptions *terraform.Options
	//nolint:gochecknoglobals // test suite state
	kindClusterName = "kind-argo-cd-test"
)

// TestArgoCdSuite bootstraps argo-cd terraform module test, with Terratest.
func TestArgoCdSuite(t *testing.T) {
	RegisterFailHandler(Fail)


	RunSpecs(t, "ArgoCd Chart test suite")
}
...
var _ = BeforeSuite(func() {
	...
	// Apply OpenTofu configuration to deploy argo-cd
	GinkgoT().Logf("Deploying argo-cd to cluster...")
	...
	Expect(os.WriteFile(chartYAMLFile, []byte(chartYAML), 0o644)).To(Succeed())
})
