...
var _ = Describe("ArgoWorkflows Deployment", func() {
	var (
	...
	BeforeEach(func() {
		...
		namespace = "kube-system" // Default namespace for argo-workflows
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "argo-workflows", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("argo-workflows"))
			}),
		...
		It("should use correct argo-workflows version from package.json", func() {
			// Verify argo-workflows version is set
			Expect(terraformOptions.Vars).To(HaveKey("argo_rollouts_version"))
			version := terraformOptions.Vars["argo_rollouts_version"]
		...
		DescribeTable("should validate pod properties",
			...
			Entry("has pods running", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				...
				pods, err := cs.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{
					LabelSelector: "app.kubernetes.io/part-of=argo-workflows",
				})
				Expect(err).NotTo(HaveOccurred())
