...
var _ = Describe("Tekton Dashboard Deployment", func() {
	var (
	...
	BeforeEach(func() {
		...
		namespace = "kube-system" // Default namespace for tekton-dashboard
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "tekton-dashboard", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("tekton-dashboard"))
			}),
		...
		It("should use correct tekton-dashboard version from package.json", func() {
			// Verify tekton-dashboard version is set
			Expect(terraformOptions.Vars).To(HaveKey("tekton-dashboard_version"))
			version := terraformOptions.Vars["tekton-dashboard_version"]
		...
		DescribeTable("should validate pod properties",
			...
			Entry("has pods running", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				...
				pods, err := cs.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{
					LabelSelector: "app.kubernetes.io/part-of=tekton-dashboard",
				})
				Expect(err).NotTo(HaveOccurred())
