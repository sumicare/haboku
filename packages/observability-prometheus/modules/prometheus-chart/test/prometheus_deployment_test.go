...
var _ = Describe("Prometheus Deployment", func() {
	var (
	...
	It("chart YAML matches golden file", func() {
		Expect(chartYAML).NotTo(BeEmpty())

		goldieFixtures := goldie.New(testT,
			goldie.WithFixtureDir("testdata"),
			goldie.WithNameSuffix(".golden.yaml"),
		)

		goldenFile := filepath.Join("testdata", "prometheus-chart.golden.yaml")
		if _, err := os.Stat(goldenFile); os.IsNotExist(err) {
			GinkgoT().Logf("Golden file not found, creating: %s", goldenFile)
			Expect(goldieFixtures.Update(testT, "prometheus-chart", []byte(chartYAML))).To(Succeed())
		} else {
			goldieFixtures.Assert(testT, "prometheus-chart", []byte(chartYAML))
		}
	})
})
