...
resource "docker_image" "nats" {
  name         = "${var.repository}${var.org}/nats:${var.nats_version}"
  keep_locally = true
  triggers = {
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/nats:${var.nats_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "nats" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.nats.name
  keep_remotely = true
