...
var _ = Describe("ArgoCD Deployment", func() {
	...
	BeforeEach(func() {
		...
		namespace = "kube-system" // Default namespace for argo-rollouts
	...
	Describe("Deployment validation", func() {
		DescribeTable("should validate deployment properties",
			...
			Entry("deployment exists", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				deployment, err := cs.AppsV1().Deployments(ns).Get(ctx, "argo-rollouts", metav1.GetOptions{})
				Expect(err).NotTo(HaveOccurred())
				Expect(deployment).NotTo(BeNil())
				Expect(deployment.ObjectMeta.Name).To(Equal("argo-rollouts"))
			}),
		...
		It("should use correct argo-rollouts version from package.json", func() {
			// Verify argo-rollouts version is set
		...
		DescribeTable("should validate pod properties",
			...
			Entry("has pods running", func(cs *kubernetes.Clientset, ctx context.Context, ns string) {
				...
				pods, err := cs.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{
					LabelSelector: "app.kubernetes.io/part-of=argo-rollouts",
				})
				Expect(err).NotTo(HaveOccurred())
