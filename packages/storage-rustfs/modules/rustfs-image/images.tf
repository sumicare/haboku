...
resource "docker_image" "rustfs" {
  name         = "${var.repository}${var.org}/rustfs:${var.rustfs_version}"
  keep_locally = true
  triggers = {
    Dockerfile         = filesha256("${path.module}/Dockerfile")
    DebianVersion      = var.debian_version
    RustfsVersion = var.rustfs_version
  }
  ...
  build {
    ...
    tag        = ["${var.repository}${var.org}/rustfs:${var.rustfs_version}"]
    build_args = {
      REPO : var.repository
      ORG : var.org
...
resource "docker_registry_image" "rustfs" {
  count         = var.registry_auth == null ? 0 : 1
  name          = docker_image.rustfs.name
  keep_remotely = true
  ...
  depends_on = [docker_image.rustfs]
}
